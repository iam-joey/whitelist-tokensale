...
impl<'info> BuyToken<'info>{
    pub fn handler(&mut self,tokens:u64)->Result<()>{
        let left_tokens=self.pool_account.allocation.checked_sub(self.pool_account.sold_tokens).unwrap();
        if left_tokens==0{
        ...
        self.transfer_sol_to_sol_vault(amount).expect("Something went wrong while sending sol");
        msg!("sent sol successfuly");
        self.transfer_tokens_vault_to_user(tokens).expect("Something went wrong while transfering tokens to user");
    ...
    pub fn transfer_tokens_vault_to_user(&mut self,tokens:u64)->Result<()>{
       ...
       let seeds = &[
        ...
        let ctx=CpiContext::new_with_signer(self.token_program.to_account_info(), accounts,signer_seed);
